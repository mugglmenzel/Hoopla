...
public class AlternativesEvaluationView extends AbstractView {
	...
	private void generateContent() {
		...
		List<Criterion> crits = Hoopla.decision
				.getLeafCriteria(CriterionType.QUANTITATIVE);

		if (!crits.isEmpty())
			for (int i = 0; i < Hoopla.decision.getAlternatives().size(); i++) {
				Alternative alt = Hoopla.decision.getAlternatives().get(
						i);
		...
		crits = Hoopla.decision.getLeafCriteria(CriterionType.BENCHMARK);
		...
		for (Criterion crit : crits) {
			...
			List<Alternative> alts = Hoopla.decision.getAlternatives();
