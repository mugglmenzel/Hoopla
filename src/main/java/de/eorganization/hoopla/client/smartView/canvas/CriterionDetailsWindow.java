...
import de.eorganization.hoopla.client.validators.CriterionNameValidator;
...
public class CriterionDetailsWindow extends Window {
	...
	public CriterionDetailsWindow(String goalName, String criterionName,
		...
		this.critTree = critTree;
		Criterion crit = Hoopla.decision.getGoal(goalName).getCriterion(
				criterionName);
		Criterion parentCrit;
		if (!goalName.equals(parentCriterionName)) {
			parentCrit = Hoopla.decision.getGoal(goalName).getCriterion(
					parentCriterionName);
		} else {
			parentCrit = Hoopla.decision.getGoal(goalName);
		}
		createWindowLayout(goalName);
			...
			saveCriterionButton.addClickHandler(new ClickHandler() {
				public void onClick(ClickEvent event) {
					if (form.validate()) {
						...
						Criterion parentCriterion = Hoopla.decision
	...
	public LinkedHashMap<String, String> updateParentCriterionItem(
		...
		if (Hoopla.decision.getGoals().size() > 0) {
			Goal goal = Hoopla.decision.getGoal(goalName);
	...
	public LinkedHashMap<String, String> updateParentCriterionItemForEdit(
		...
		Goal currentGoal = Hoopla.decision.getGoal(goalName);
