...
import de.fzi.aotearoa.client.validators.GoalNameValidator;
...
public class GoalsAndCriteriaCanvas extends Canvas {
	...
	private Window createGoalDetailsWindow(final String goalName) {
		...
		if (goalName != null) {
			final Goal goal = AotearoaSmart.decision.getGoal(goalName);
	...
	private void refreshGoalsTabSet() {
		...
		List<Goal> allGoals = AotearoaSmart.decision.getGoals();
		if (allGoals.size() > 0) {
				...
				SC.confirm("Do you really want to delete " + goalName
						...
						new BooleanCallback() {
							public void execute(Boolean value) {
								if (value != null && value) {
									AotearoaSmart.decision.deleteGoal(goalName);
									refresh();
									handleButtons();
								}
							}
