...
public class CriteriaEvaluationView extends AbstractView {
	...
	public CriteriaEvaluationView() {
		...
		getPostit()
				.setContents(
						"Aotearoa supports defining a specific importance for all goals and criteria to reflect the complexity of a Cloud decision precisely. To support the user with the definition of the decision's importance preferences Aotearoa uses pairwise comparisons.");
	...
	private void generateContent() {
		...
		List<List<Criterion>> criteriaByLevel = AotearoaSmart.decision
				.getCriteriaByLevels();
		...
		List<GoalImportance> goalImportances = AotearoaSmart.decision.getImportanceGoals();
		...
		if (goals.size() > 1)
			for (int i = 0; i < goals.size() - 1; i++)
				for (int j = i + 1; j < goals.size(); j++) {
					...
					GoalImportance imp = AotearoaSmart.decision
							.getImportanceGoal(i, j);

					if (imp == null) {

						imp = new GoalImportance(i, j, 0D, "");
						AotearoaSmart.decision.insertImportanceGoal(imp);
					...
					hSlider.addValueChangedHandler(new ValueChangedHandler() {
						...
						public void onValueChanged(ValueChangedEvent event) {
							...
							AotearoaSmart.decision.insertImportanceGoal(
									importance);
						}
					});
		...
		for (List<Criterion> leafs : leafsLists) {
			...
			crits.removeAll(AotearoaSmart.decision.getGoals());
			if (crits.size() > 1)
				for (int i = 0; i < crits.size() - 1; i++)
					for (int j = i + 1; j < crits.size(); j++) {
						...
						CriterionImportance imp = AotearoaSmart.decision
