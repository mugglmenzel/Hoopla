...
import de.fzi.aotearoa.client.datasource.CriteriaDataSource;
...
//			 AotearoaSmart.decision.getCriterion( ((CriterionTreeNode)
//			 event.getFolder()) .getAttribute("Name")).addChild(
//			 AotearoaSmart.decision .getCriterion(((CriterionTreeNode) node).getAttribute("Name"))); } 
			 refreshCriteriaTree();
	...
	public void refreshCriteriaTree() {
		if (!AotearoaSmart.decision.getGoals().isEmpty()) {
			final Tree tree = new Tree();
			tree.setModelType(TreeModelType.PARENT);
			tree.setNameProperty("Name");
			tree.setIdField("Name");
